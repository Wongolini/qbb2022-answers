...
my_data = np.genfromtxt(sys.argv[2],skip_header=1)

compare_paper_model = np.zeros((len(accession),2))
ordered_acc = np.argsort(accession)
compare_paper_model[:,0] = np.array(k562_model_predictions)[ordered_acc] # model predictions
compare_paper_model[:,1] = my_data[:,1][ordered_acc] # my models predictions

fig, ax = plt.subplots()
x, y = compare_paper_model[:,0], compare_paper_model[:,1]
...
ax.set_xlabel("Paper Predicted K562 expression level,\n10-fold cross-validated")
ax.set_ylabel("Nicholas Predicted K562 expression level (log10)")
corr = pearsonr(k562_model_predictions, k562_observations)
ax.text(.5, 3.75, "r^2 = " + str(round(corr[0]**2,2)) + "\nn = " + str(len(k562_observations)))
line_xs = np.linspace(max(min(k562_model_predictions), min(k562_observations)), min(max(k562_model_predictions), max(k562_observations)), 100)
line_ys = 0 + 1 * line_xs
ax.plot(line_xs, line_ys, color = "maroon")
